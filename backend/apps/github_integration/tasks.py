...
@shared_task(bind=True, max_retries=3)
def import_public_repo_task(self, project_id, full_name, folder_path, branch, description, custom_info):
    """Import from a public GitHub repo without user OAuth token."""
    try:
        from .fetcher import import_public_folder_from_repo

        project = Project.objects.get(id=project_id)

        files = import_public_folder_from_repo(full_name, folder_path, branch)

        if not files:
            project.status = Project.Status.FAILED
            project.error_message = "No Python files found or repository is private"
        ...
        logger.exception(f"import_public_repo_task failed for project {project_id}: {e}")
        try:
